...
/*
    ...
    3、说明
       3.1 关于权限修饰符： 默认方法的权限修饰符都先使用public
            Java规定的4中权限修饰符： private、public、缺省、protected  --》封装性再细分
       3.2 返回值类型： 有返回值 vs 无返回值
            ① 如果方法有返回值，则必须再方法声明时，指定返回值的类型。同时
               方法中，需要使用return 关键字来返回指定类型的变量或常量
            ② 如果方法没有返回值，则方法声明时，使用void来表示。通常，没有
               返回值的方法中，就不使用return。但是，如果使用的话，只能"return;"
               表示结束此方法的意思。
...
class Customer{
    ...
    public void Sort(){
        int[] arr = new int[]{3, 4, 5, 6, 7, 8, 1, 2, 33, 46};
